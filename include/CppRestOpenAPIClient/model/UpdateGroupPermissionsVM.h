/**
 * FastReport Cloud
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: v1
 *
 * NOTE: This class is auto generated by OpenAPI-Generator 7.3.0.
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * UpdateGroupPermissionsVM.h
 *
 * 
 */

#ifndef FASTREPORT_CLOUD_MODELS_UpdateGroupPermissionsVM_H_
#define FASTREPORT_CLOUD_MODELS_UpdateGroupPermissionsVM_H_


#include "CppRestOpenAPIClient/model/GroupPermissionsCRUDVM.h"
#include "CppRestOpenAPIClient/model/GroupAdministrate.h"
#include <cpprest/details/basic_types.h>
#include "CppRestOpenAPIClient/model/CloudBaseVM.h"

namespace fastreport {
namespace cloud {
namespace models {

class GroupPermissionsCRUDVM;

/// <summary>
/// 
/// </summary>
class  UpdateGroupPermissionsVM
    : public CloudBaseVM
{
public:
    UpdateGroupPermissionsVM();
    virtual ~UpdateGroupPermissionsVM();

    /////////////////////////////////////////////
    /// ModelBase overrides

    void validate() override;

    web::json::value toJson() const override;
    bool fromJson(const web::json::value& json) override;

    void toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) const override;
    bool fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& namePrefix) override;

    /////////////////////////////////////////////
    /// UpdateGroupPermissionsVM members

    /// <summary>
    /// 
    /// </summary>
    std::shared_ptr<GroupPermissionsCRUDVM> getNewPermissions() const;
    bool newPermissionsIsSet() const;
    void unsetnewPermissions();

    void setNewPermissions(const std::shared_ptr<GroupPermissionsCRUDVM>& value);

    /// <summary>
    /// 
    /// </summary>
    std::shared_ptr<GroupAdministrate> getAdministrate() const;
    bool administrateIsSet() const;
    void unsetadministrate();

    void setAdministrate(const std::shared_ptr<GroupAdministrate>& value);


protected:
    std::shared_ptr<GroupPermissionsCRUDVM> m_newPermissions;
    bool m_newPermissionsIsSet;
    std::shared_ptr<GroupAdministrate> m_administrate;
    bool m_administrateIsSet;
};


}
}
}

#endif /* FASTREPORT_CLOUD_MODELS_UpdateGroupPermissionsVM_H_ */
